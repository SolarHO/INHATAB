...
class _PostDetailPageState extends State<PostDetailPage> {
  ...
  void initState() {
    ...
    Provider.of<PostModel>(context, listen: false)
        ...
        .then((_) async {
      String? writerId =
          Provider.of<PostModel>(context, listen: false).writerId;
      if (writerId != null) {
        writerStatus = await Provider.of<PostModel>(context, listen: false)
            .fetchWriterStatus(writerId); //작성자 상태가 탈퇴된 사용자인지 구분
        setState(() {}); // 상태 업데이트
      }
    });
  ...
  void _showChatConfirmationDialog(String postUserId) {
    //채팅 다이얼로그
    showDialog(
      context: context,
      builder: (context) {
  ...
  Future<void> _startChatWithUser(String postUserId) async {
    try {
      ...
      await chatModel.startChatWithUser(
          postUserId, selectedBoard, widget.postId, context);
    } catch (error) {
  ...
  void _showCommentChatDialog(String commentUserId, bool isAnonymous) {
    print(
        "Comment User ID: $commentUserId, Is Anonymous: $isAnonymous"); // 디버깅 정보 추가
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
  ...
  Future<void> _startChatWithCommentUser(
      String commentUserId, bool isAnonymous) async {
    try {
      final chatModel = Provider.of<ChatModel>(context, listen: false);
      await chatModel.startChatFromComment(
          commentUserId, widget.postId, context, isAnonymous);
    } catch (error) {
      print('채팅방 생성 중 오류 발생: $error');
    }
  }

  Future<void> _startChatWithReplyUser(
      String replyUserId, bool isAnonymous) async {
    try {
      final chatModel = Provider.of<ChatModel>(context, listen: false);
      await chatModel.startChatFromComment(
          replyUserId, widget.postId, context, isAnonymous);
    } catch (error) {
      print('채팅방 생성 중 오류 발생: $error');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title:
              Text(Provider.of<BoardModel>(context).selectedBoard.toString()),
          actions: <Widget>[
            Visibility(
                      ...
                      return AlertDialog(
                        ...
                        actions: <Widget>[
                          ...
                          TextButton(
                            child: Text('확인'),
                            onPressed: () async {
                              await Provider.of<PostModel>(context,
                                      listen: false)
          ...
          children: [
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            Provider.of<BoardModel>(context, listen: false)
                                .updateLikeCount(postModel.PostId!,
            ...
            Container(
              decoration: BoxDecoration(
                color: Color.fromARGB(255, 207, 207, 207), // 배경색을 회색으로 설정
                borderRadius: BorderRadius.circular(10), // 테두리에 둥근 모서리 추가
              ),
              child: Padding(
                    ...
                    Expanded(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: '댓글을 입력하세요.',
                          border: InputBorder.none, // 아래 구분선 제거
                          suffixIcon: IconButton(
                            ...
                            onPressed: () {
                              ...
                              if (commentText.isNotEmpty) {
                                Provider.of<CommentModel>(context,
                                        listen: false)
                                    .addCommentToDb(
                                        _commentController.text, _isAnonymous);
                                Provider.of<BoardModel>(context, listen: false)
                                    .incCommentCount(widget.postId);
              ...
              builder: (context, commentModel, child) {
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    // 디버깅 로그 추가
                    print(
                        "댓글 인덱스: $index, 전체 댓글 수: ${commentModel.commentContents.length}");
                    print(
                        "익명 여부 인덱스: $index, 익명 여부 배열 크기: ${commentModel.isAnonymous.length}");
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ListTile(
                                ...
                                style: TextStyle(
                                  ...
                                  fontWeight:
                                      commentModel.userIds[index] != 'unknown'
                                          ? FontWeight.bold
                                          : null,
                                  color: commentModel.userIds[index] ==
                                          commentModel.postWriter
                                      ? Colors.green
                                      : null,
                                ),
                              ),
                            ...
                            return Padding(
                              ...
                              child: ListTile(
                                title: Row(
                                  children: [
                                    Text(
                                        commentModel.replies[index][replyIndex]
                                            ['replyName']!,
                                        style: TextStyle(
                                          fontWeight: FontWeight.bold,
                                          fontSize: 15,
                                          color: commentModel.replies[index]
                                                          [replyIndex]
                                                      ['replyuid'] ==
                                                  commentModel.postWriter
                                              ? Colors.green
                                ...
                                subtitle: Text(commentModel.replies[index]
                                    [replyIndex]['replyContent']!),
                                trailing: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Visibility(
        ...
        TextButton(
          ...
          onPressed: () {
            ...
            if (replietext.isNotEmpty) {
              Provider.of<CommentModel>(context, listen: false).addReplyToDb(
                  widget.commentId, replietext, _isReplyAnonymous);
              Provider.of<BoardModel>(context, listen: false)
                  .incCommentCount(widget.postId);
              Provider.of<CommentModel>(context, listen: false).clear();
              Provider.of<CommentModel>(context, listen: false)
